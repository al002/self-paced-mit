...
lowercase letters, so you will have to handle uppercase and mixed case strings
    ...
    word_len = len(word)
    for s in word:
        score += SCRABBLE_LETTER_VALUES[s.lower()]

    penalizes = 7 * word_len - 3 * (n - word_len)
    second_component = penalizes if penalizes > 1 else 1
...
def substitute_hand(hand, letter):
    ...
    user)
    with a new letter chosen from the VOWELS and CONSONANTS at random. The new
    letter
